...
- (void)systemDidWake:(NSNotification *)note {
    // Clear stale in-process state. If blacked out before sleep and the
    // external is gone, restore the built-in now — no callback will come.
    // If the external is still present, its re-announcement callback will
    ...
    BOOL wasBlackedOut = [_displayController invalidateDisplayState];
    if (wasBlackedOut && ![_displayController hasActiveExternalDisplay]) {
        NSLog(@"[wake] — was blacked out, no external display — disabling blackout");
