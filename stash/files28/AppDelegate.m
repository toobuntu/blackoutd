...
static NSString * const kSuiteName         = @BD_BUNDLE_ID ".prefs";
...
- (void)waitForWindowServerThenRestoreState {
    ...
    int __block token = 0;
    notify_register_dispatch("com.apple.windowserver.active",
                             &token,
...
- (void)systemWillSleep:(NSNotification *)note {
    NSLog(@"[sleep] — suppressing display callbacks");
    _displayController.suppressCallbacks = YES;
}

- (void)systemDidWake:(NSNotification *)note {
    NSLog(@"[wake] — restoring display callbacks, re-evaluating state");
    _displayController.suppressCallbacks = NO;
    [_displayController invalidateDisplayState];
    [self reapplyPersistedIntent];
}

// Re-applies blackout intent from persisted defaults after wake, treating
// _isBlackedOut as stale. Reads kBlackoutActiveKey as the source of truth.
- (void)reapplyPersistedIntent {
    BOOL intent = [_defaults boolForKey:kBlackoutActiveKey];
    if (intent && [_displayController hasActiveExternalDisplay]) {
        NSLog(@"[wake] — intent=blackout hasExternal=1 — re-applying blackout");
        [_displayController enableBlackout];
    } else if (intent && ![_displayController hasActiveExternalDisplay]) {
        NSLog(@"[wake] — intent=blackout hasExternal=0 — clearing stale intent, restoring");
        [_defaults setBool:NO forKey:kBlackoutActiveKey];
        [_displayController disableBlackout];
    } else {
        NSLog(@"[wake] — intent=restore — no action");
    }
}
...
- (void)setupSignalHandlers {
    ...
    _sighupSource = [self dispatchSourceForSignal:SIGHUP handler:^{
        [self reloadPreferences];
    }];
    _sigtermSource = [self dispatchSourceForSignal:SIGTERM handler:^{
        [NSApp terminate:nil];
    }];
    _sigintSource = [self dispatchSourceForSignal:SIGINT handler:^{
        [NSApp terminate:nil];
    }];
}
