...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
    ...
    NSLog(@"[startup] — verbosityLevel=%ld", (long)_displayController.verbosityLevel);
    [self setupMenuBar];
...
- (void)waitForWindowServerThenRestoreState {
    ...
    int __block token = 0;
    notify_register_dispatch("com.apple.windowserver.active", &token, dispatch_get_main_queue(),
                             ^(int t) {
                                 notify_cancel(t);
                                 [self restorePersistedState];
                             });
...
- (void)bootoutAndQuit {
    NSString *domain = [NSString stringWithFormat:@"gui/%d/%@", getuid(), kAgentLabel];
...
- (void)setupMenuBar {
    ...
    _autoItem = [[NSMenuItem alloc] initWithTitle:@"Auto-blackout on External Connect"
                                           action:@selector(toggleAutoBlackout:)
                                    keyEquivalent:@""];
    _autoItem.target = self;
    _autoItem.state = _displayController.autoBlackoutOnExternalConnect ? NSControlStateValueOn
                                                                       : NSControlStateValueOff;
    [menu addItem:_autoItem];

    [menu addItem:NSMenuItem.separatorItem];

    NSMenuItem *quitItem = [[NSMenuItem alloc] initWithTitle:@"Quit blackoutd"
...
- (void)updateMenuBarIcon {
    NSString *symbol = _displayController.isBlackedOut ? @"macbook.slash" : @"macbook";
...
- (void)toggleAutoBlackout:(id)sender {
    ...
    if (newValue) {
        ...
        if (!_displayController.isBlackedOut && [_displayController hasActiveExternalDisplay]) {
            [_displayController enableBlackout];
        }
    } else {
...
- (void)setupSignalHandlers {
    ...
    _sigusr1Source = [self dispatchSourceForSignal:SIGUSR1
                                           handler:^{
                                               [self->_displayController enableBlackout];
                                           }];
    _sigusr2Source = [self dispatchSourceForSignal:SIGUSR2
                                           handler:^{
...
- (dispatch_source_t)dispatchSourceForSignal:(int)sig handler:(dispatch_block_t)handler {
    dispatch_source_t source =
        dispatch_source_create(DISPATCH_SOURCE_TYPE_SIGNAL, sig, 0, dispatch_get_main_queue());
